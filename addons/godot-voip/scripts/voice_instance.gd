...
func _process_mic():
	if recording:
		...
		if stereo_data.size() > 0:
			...
			var type_chat = get_tree().get_root().get_node("World").is_private
			
			if !type_chat:
				var room_chat = get_tree().get_root().get_node("World/MenuChat").room_chat
				rpc_unreliable_id(1, "_speak", data,  get_tree().get_network_unique_id(), room_chat)
			else: 
				var id_receiver = int(get_tree().get_root().get_node("World/RPCClient").id_receiver)
				rpc_unreliable_id(1, "receive_private_audio", data,  get_tree().get_network_unique_id(), id_receiver)
